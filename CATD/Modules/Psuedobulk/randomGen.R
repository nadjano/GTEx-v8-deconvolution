...
'2'={
    ...
    T_P <- foreach (i=1:nSamples, .combine = combFunc) %dopar%
    {
      ...
      {
        P <- abs(rbinorm(length(unique(T_prep@meta.data$cellType)), 0, 1, 0.0001, propVar, 0.5))
      ...
      else if (propVar <= 0)
      {
        ...
        P <- runif(length(unique(T_prep@meta.data$cellType)), 1, 99)
      }

      else
      {
        #Fit the distribution with given variance
        P <- round(runif(length(unique(T_prep@meta.data$cellType)), 100, 100 + (propVar*2)))/100
      }
